...
actual class DatabaseReference internal constructor(
    ...
    actual suspend fun updateChildren(update: Map<String, Any?>): Unit = suspendCancellableCoroutine { cont ->
        ios.updateChildValues(update as Map<Any?, *>) { error, _ ->
            if (error != null) cont.resumeWithException(Exception(error.localizedDescription))
            else cont.resume(Unit)
        }
    }
    ...
    // onDisconnect no está disponible como propiedad en iOS - crear wrapper
    actual fun onDisconnect(): OnDisconnect = OnDisconnect(ios)
}
...
actual class DataSnapshot internal constructor(val ios: FIRDataSnapshot) {
    ...
    actual val children: Iterable<DataSnapshot> get() {
        ...
        val enumerator = ios.children
        var next = enumerator.nextObject()
        while (next != null) {
            result.add(DataSnapshot(next as FIRDataSnapshot))
...
actual class OnDisconnect internal constructor(private val ios: FIRDatabaseReference) {
    actual suspend fun setValue(value: Any?) { 
        ios.onDisconnectSetValue(value) 
    }
    
    @Suppress("UNCHECKED_CAST")
    actual suspend fun updateChildren(update: Map<String, Any?>) { 
        ios.onDisconnectUpdateChildValues(update as Map<Any?, *>) 
    }
    
    actual suspend fun removeValue() { 
        ios.onDisconnectRemoveValue() 
    }
    
    actual suspend fun cancel() { 
        ios.cancelDisconnectOperations() 
    }
}
